...
import { PrototypeStorageService } from '../services/prototype-storage.service';
...
export class MockCustomerRepository implements CustomerRepository {
  private readonly storage = inject(PrototypeStorageService);
  private readonly customerState = signal<Customer[]>(
    this.storage.loadCustomers() ?? createInitialCustomers(),
  );

  readonly customers = this.customerState.asReadonly();

  updateCustomers(updater: CustomerUpdater): void {
    const updatedCustomers = updater(this.customerState());
    this.customerState.set(updatedCustomers);
    this.storage.saveCustomers(updatedCustomers);
  }

  resetCustomers(): void {
    this.customerState.set(createInitialCustomers());
    this.storage.clearCustomers();
  }
}
...
function createInitialCustomers(): Customer[] {
  return INITIAL_CUSTOMERS.map((customer) => ({ ...customer }));
}
